...
```bash
python -m pytest tests/ -q        # 256 passed aana chahiye
...
- [ ] `.env` mein `GEMINI_API_KEY` daala ho.
...
**Agar `GEMINI_API_KEY` nahi hai:** "Run live" button kaam nahi karega. Bas itna bol do —
...
**Agar `GEMINI_API_KEY` nahi hai:** ye kaam nahi karega, error aayega. Tab ye bolo —
