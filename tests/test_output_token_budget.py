...
OTPM_ERROR = (
    ...
    "`gemini-3.8-flash` in organization `org_01m1gjj1caehvr85vgz1av492h` service tier "
...
ORDINARY_RATE_LIMIT = (
    ...
    "`gemini-3.8-flash`. Please try again in 12.5s.', 'code': 'rate_limit_exceeded'}}"
)
...
def test_ordinary_rate_limit_still_backs_off(main):
    pool = main.KeyPool.__new__(main.KeyPool)
    pool._dead_until = {}
    assert main._handle_error(pool, "GEMINI_API_KEY", ORDINARY_RATE_LIMIT) == pytest.approx(12.5)
...
OTPM_1000 = (
    "Error code: 429 - {'error': {'message': \"Request too large for model "
    "`gemini-3.8-flash` in organization `org_x` service tier `on_demand` on output "
...
def test_first_otpm_rejection_resizes_and_retries_immediately(main):
    pool = _DummyPool()
    assert main._handle_error(pool, "GEMINI_API_KEY", OTPM_1000) == 0
...
def test_repeat_rejection_honours_an_explicit_retry_hint(main):
    ...
    assert main._handle_error(pool, "GEMINI_API_KEY", hinted) == 12.5
...
def test_a_pinned_ceiling_still_backs_off_rather_than_spinning(main, monkeypatch):
    ...
    assert mod._handle_error(_DummyPool(), "GEMINI_API_KEY", OTPM_1000) >= 30


def test_model_is_overridable_without_editing_code(monkeypatch):
    monkeypatch.setenv("AEDI_MODEL", "gemini-3.8-pro")
    assert _load_fresh().MODEL == "gemini-3.8-pro"
